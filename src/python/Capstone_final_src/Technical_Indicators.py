...
class technical_indicators():
    ...
    def stma_cuts_ltma(self, stock):        
        ...
        self.stock['stma_pr_cut_ltma_a'] = values2

        # stma overlaps ltma
        self.stock['stma_overlaps_ltma'] = overlapping
    ...
    def close_cuts_stma(self, stock):        
        ...
        # close price overlaps stma
        self.stock['close_pr_overlaps_stma'] = overlapping
    ...
    def close_cuts_ltma(self, stock):
        ...
        for k in range(0, len(self.stock)):                
            ...
            if close[i] == ltma[j]: # point of intersection  
                overlapping.append(1)
                ...
                elif (close[i-1] > ltma[j-1]):  
                    values2.append(1) # close price cuts ltma from above
                    values1.append(0) # close price cuts ltma from below                

                elif (close[i-1] < ltma[j-1]):         
                    values1.append(1) # close price cuts ltma from below
                    values2.append(0) # close price cuts ltma from above
        ...
        # close price overlaps ltma
        self.stock['close_pr_overlaps_ltma'] = overlapping
    ...
    def vwap(self, stock):    
        ...
        volume = self.stock['Volume']
        
        """ There is error in this function.         
        Get all the different volumes and their respective prices and then calculate volume weighted average price.
        OR else get the Vwap values for each self.stock from nse.py package directly"""

        # To handle 'ZeroDivisionError'    
        volume = volume.replace(to_replace={0:np.nan})
        vwap = round(((price * volume) / volume), 2)
        self.stock['vwap'] = vwap
        return self.stock
    
    
   
    # Price movement (up/down)
    def price_movement(self, stock):
        self.stock = stock
    ...
    def beta_values(self, stock):        
        ...
        self.stock['beta'] = round((dr_stock / dr_NSE), 4)
        return self.stock
